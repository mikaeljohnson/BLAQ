...
contract OnesSource is Context, IERC20, IERC20Metadata {
     ...
     */
    constructor() {
        // _name = "Ones Source"; // TODO uncomment these with others
        // _symbol = "BLAQ";

        _name = "fakename"; // TODO uncomment these with others
        _symbol = "CUMCOIN";

        _mint(msg.sender, 7000000000000000000000); // All gets sent to presale address after launch. 20000000000000000000000000 goes to liquidity and 50000000000000000000000000 will be sold in presale.
        _mint(0xC28e3559298DB2D54B36D62A130cB664d1B2A77e, 3000000000000000000000); // treasuryAddress
...
contract BlaqPresale is Ownable {
    ...
    IERC20 public _blaq =        IERC20(0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174);
    ...
    address private _devWalletOne =     0x641E8274ce7513e2df215FcAD97515165019C497; // TODO make this proper value
    address private _devWalletTwo =     0x780e4a353fE793f053E8F67f43276f0Bc839831c;
    ...
    uint256 public _totalPresaleTokens = 5000; //TODO fix these 2
    uint256 public _totalLiquidityTokens = 2000;
    ...
    constructor(address blaq) {
        _blaq = IERC20(blaq);
        _router = IUniswapV2Router01(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506);

        // TODO uncomment these and remove the code under
        ...
        // _rounds[1]._tokensLeft = _totalPresaleTokens / 4; //1250000
        ...
        _presaleEndDate = block.timestamp + (10 * 1 minutes);
        _rounds[1]._tokensLeft = _totalPresaleTokens / 4;
        _rounds[1]._usdcPrice = 30;
        
        _rounds[2]._tokensLeft = _totalPresaleTokens / 4;
        _rounds[2]._usdcPrice = 60;
        
        _rounds[3]._tokensLeft = _totalPresaleTokens / 4;
        _rounds[3]._usdcPrice = 90;
        
        _rounds[4]._tokensLeft = _totalPresaleTokens / 4;
        _rounds[4]._usdcPrice = 120;
    ...
    function claimable(address user) public view returns(uint256) {
        ...
        uint256 claimUnits = (block.timestamp - _presaleEndDate) / 1 minutes; // TODO make this 1 weeks
        uint256 unitsClaimed = _investors[user]._unitsClaimed;
        uint256 monthsToClaim;

        //  (condition) ? (if true x): (else y)
    ...
    function claim() external {
        require(!_isInPresale, "Vesting has not begun.");
        uint256 claimableTokens = claimable(msg.sender);
        require(claimableTokens > 0, "You have no tokens to claim at this point.");

        uint256 claimUnits = (block.timestamp - _presaleEndDate) / 1 minutes;
        _investors[msg.sender]._unitsClaimed = (claimUnits < 32) ? claimUnits : 32; // TODO make sure _investors shows 32 when max claiming and maybe fix for claiming less time??
